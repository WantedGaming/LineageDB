<?php
// view_armor.php - Detailed view of a specific armor item
require_once 'database.php';

// Page setup
$page_title = "Armor Details";
include 'header.php';
...
$statCategories = [
    ...
    'Defense' => [
        ...
        'MagicDamageReduction' => 'Damage Reduction (Magic)'
    ],
    'Elemental Resist' => [
        'defense_water' => 'Water',
        'defense_wind' => 'Wind',
...
if ($itemId <= 0) {
    ...
    <div class="container mt-4">
        <div class="alert alert-danger">Invalid Armor ID</div>
    </div>
    <?php
    include 'footer.php';
    exit();
...
try {
    ...
    if ($result->num_rows === 0) {
        ?>
        <div class="container mt-4">
            <div class="alert alert-warning">No armor found with ID <?php echo $itemId; ?></div>
        </div>
        <?php
        include 'footer.php';
        exit();
    ...
    $armor['material'] = cleanText($armor['material']);
    $armor['desc_en'] = cleanText($armor['desc_en']);
    ...
    foreach ($statCategories as $category => $stats) {
        ...
        foreach ($stats as $statKey => $statLabel) {
            // Special handling for 'ac' and 'ac_sub' to always show
            if (($statKey === 'ac' || $statKey === 'ac_sub') || 
                (isset($armor[$statKey]) && $armor[$statKey] != 0)) {
    ...
    foreach ($classMapping as $key => $className) {
        if (isset($armor[$key]) && $armor[$key] == 1) {
            $classRestrictions[] = $className;
        }
    }

    // Prepare icon path
    $iconPath = "icons/{$armor['iconId']}.png";
    $hasIcon = !empty($armor['iconId']) && file_exists($iconPath);
...
<div class="container mt-4">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <table class="table table-sm table-borderless">
                                <tr>
                                    <th>Item ID</th>
                                    <td><?php echo htmlspecialchars($armor['item_id']); ?></td>
                                </tr>
                                <tr>
                                    <th>Grade</th>
                                    <td>
                                        <span class="badge grade-<?php echo htmlspecialchars($armor['itemGrade']); ?>">
                                            <?php echo htmlspecialchars($armor['itemGrade']); ?>
                                        </span>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Type</th>
                                    <td><?php echo htmlspecialchars($armor['type']); ?></td>
                                </tr>
                                <tr>
                                    <th>Material</th>
                                    <td><?php echo htmlspecialchars($armor['material']); ?></td>
                                </tr>
                            </table>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <table class="table table-sm table-borderless">
                                <tr>
                                    <th>Level</th>
                                    <td><?php echo htmlspecialchars($armor['grade']); ?></td>
                                </tr>
                                <tr>
                                    <th>AC</th>
                                    <td><?php echo htmlspecialchars($armor['ac']); ?></td>
                                </tr>
                                <tr>
                                    <th>Weight</th>
                                    <td><?php echo htmlspecialchars($armor['weight']); ?></td>
                                </tr>
                                <tr>
                                    <th>Safe Enchant</th>
                                    <td><?php echo htmlspecialchars($armor['safenchant']); ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
    ...
    try {
        // Prepare query to find monster drops for this item
    ...
    <div class="text-center mt-4">
        <a href="armor_list.php" class="btn btn-secondary">
            <i class="bi bi-arrow-left me-2"></i>Back to Armor List
        </a>
    </div>
</div>
