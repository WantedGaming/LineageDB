<?php
// accessory_list.php - Main page showing accessory list with advanced filters
require_once 'database.php';

$page_title = "Accessories List - Game Armor Database";
include 'header.php';

// Define accessory types for filtering
...
// Include only accessory types
$conditions[] = "type IN ('" . implode("','", $accessoryTypes) . "')";
...
if (!empty($filters['type'])) {
    ...
    foreach ($filters['type'] as $type) {
        if (in_array($type, $accessoryTypes)) {
            $typeConditions[] = "type = ?";
...
$gradesQuery = "SELECT DISTINCT itemGrade FROM armor WHERE type IN ('" . implode("','", $accessoryTypes) . "') ORDER BY CASE 
...
// Get all accessory types for filter (only from the defined accessory types)
$typesQuery = "SELECT DISTINCT type FROM armor WHERE type IN ('" . implode("','", $accessoryTypes) . "') ORDER BY type";
$typesResult = $conn->query($typesQuery);

// Get all materials for filter (only for accessory types)
$materialsQuery = "SELECT DISTINCT material FROM armor WHERE material != 'NONE(-)' AND type IN ('" . implode("','", $accessoryTypes) . "') ORDER BY material";
        ...
        <div class="filter-sidebar mb-4">
            <h5 class="filter-header">
                ...
                <?php if(array_filter($filters, function($v) { return !empty($v) && $v !== 'item_id' && $v !== 'ASC'; })): ?>
                    <a href="accessory_list.php" class="btn btn-sm btn-outline-danger float-end" title="Reset filters">
                        <i class="bi bi-x-lg"></i>
                    </a>
                <?php endif; ?>
            </h5>
            <form id="filterForm" method="get" action="accessory_list.php">
                <!-- Search Filter -->
    ...
    <div class="col-lg-9">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    <i class="bi bi-gem me-2"></i>Accessories List
                    <span class="text-muted small ms-2">(<?php echo number_format($totalItems); ?> items)</span>
                </h5>
                <a href="armor_list.php" class="btn btn-outline-primary btn-sm">
                    <i class="bi bi-shield-fill me-1"></i>View Armor
                </a>
            </div>
                            ...
                            <thead>
                                <tr>
                                    <th class="px-3 text-center">
                                        <a href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['sort', 'order']); ?>&sort=iconId&order=<?php echo $sortColumn == 'iconId' && $sortOrder == 'ASC' ? 'DESC' : 'ASC'; ?>" class="text-decoration-none">
                                            Icon
                                    ...
                                    <th class="px-3">
                                        <a href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['sort', 'order']); ?>&sort=desc_en&order=<?php echo $sortColumn == 'desc_en' && $sortOrder == 'ASC' ? 'DESC' : 'ASC'; ?>" class="text-decoration-none">
                                            Name
                                    ...
                                    <th class="px-3">
                                        <a href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['sort', 'order']); ?>&sort=type&order=<?php echo $sortColumn == 'type' && $sortOrder == 'ASC' ? 'DESC' : 'ASC'; ?>" class="text-decoration-none">
                                            Type
                                    ...
                                    <th class="px-3">
                                        <a href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['sort', 'order']); ?>&sort=grade&order=<?php echo $sortColumn == 'grade' && $sortOrder == 'ASC' ? 'DESC' : 'ASC'; ?>" class="text-decoration-none">
                                            Level
                                            <?php if ($sortColumn == 'grade'): ?>
                                                <i class="bi bi-arrow-<?php echo $sortOrder == 'ASC' ? 'up' : 'down'; ?>"></i>
                                            <?php endif; ?>
                                        </a>
                                    </th>
        ...
        <tr class="clickable-row" data-href="view_armor.php?id=<?php echo $row['item_id']; ?>">
            ...
            <td class="grade-<?php echo htmlspecialchars($row['itemGrade']); ?> px-3">
                ...
                <div class="float-end">
                    ...
                    <a href="delete_armor.php?id=<?php echo $row['item_id']; ?>" class="btn btn-sm btn-outline-danger" title="Delete" onclick="return confirm('Are you sure you want to delete this accessory item?');">
                        <i class="bi bi-trash"></i>
                    </a>
                </div>
            </td>
            <td class="px-3"><?php echo htmlspecialchars($row['type']); ?></td>
            <td class="px-3"><?php echo htmlspecialchars($row['grade']); ?></td>
        </tr>
    <?php endwhile; ?>
</tbody>
                        </table>
                ...
                <?php else: ?>
                    <div class="p-4 text-center">
                        ...
                        <a href="accessory_list.php" class="btn btn-outline-secondary">Reset Filters</a>
                    </div>
                <?php endif; ?>
            </div>
            <?php if ($totalPages > 1): ?>
                <div class="card-footer">
                    <nav>
                        <ul class="pagination pagination-sm justify-content-center mb-0">
                            <?php if ($page > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['page']); ?>&page=<?php echo $page - 1; ?>">
                            ...
                            if ($startPage > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['page']); ?>&page=1">1</a>
                                </li>
                                <?php if ($startPage > 2): ?>
                            ...
                            <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                                <li class="page-item <?php echo $i == $page ? 'active' : ''; ?>">
                                    <a class="page-link" href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['page']); ?>&page=<?php echo $i; ?>">
                                        <?php echo $i; ?>
                                    </a>
                                </li>
                            <?php endfor; ?>
                            ...
                            if ($endPage < $totalPages): ?>
                                ...
                                <?php endif; ?>
                                <li class="page-item">
                                    <a class="page-link" href="accessory_list.php?<?php echo buildFilterQueryString($filters, ['page']); ?>&page=<?php echo $totalPages; ?>">
                                        <?php echo $totalPages; ?>
                                    </a>
                                </li>
                            <?php endif; ?>
